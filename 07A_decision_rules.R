...
# Apply a multi-arm causal forest (MACF) + policy tree to identify decision rules
dec_rules <- fun_MACF(dec_rules = dec_rules, 
                      num.trees = 1000, 
                      honesty = TRUE, 
                      seed = settings$seed, 
                      policysample = 1.0,
...
html_plot <- plot(dec_rules$MACF$policy_tree, leaf.labels = levels(dec_rules[["W"]]))
...
## Hybrid policy tree (depth = 3) is possible but takes very long to run
