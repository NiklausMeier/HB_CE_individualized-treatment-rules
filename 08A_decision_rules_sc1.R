...
# Task: Run scenario 1 for decision rules - Death as random event              #
...
for (i in 1:nloops){
  ...
  model <- fun_bleeds_death(model = model, mode = "random") # Very important here: Random death rather than expected value makes better treatment predictions harder
...
dec_rules_sc1 <- fun_opt_treat(data = PRC, 
...
dec_rules_sc1 <- fun_rpart(dec_rules = dec_rules_sc1, 
                           des_split = 2,
                           treatments = names(model[["sim"]]),
...
save(dec_rules_sc1, file = paste0(directories$dir_dat_deriv, '/dec_rules_sc1.Rdata'))
